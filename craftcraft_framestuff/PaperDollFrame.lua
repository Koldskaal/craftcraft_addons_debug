...
function PaperDollFrame_SetLevel()
	local secondaryClassName = "";
	if (CLASS_SECONDARY == 1) then
		secondaryClassName = " / Warrior";
	elseif (CLASS_SECONDARY == 2) then
		secondaryClassName = " / Paladin";
	elseif (CLASS_SECONDARY == 3) then
		secondaryClassName = " / Hunter";
	elseif (CLASS_SECONDARY == 4) then
		secondaryClassName = " / Rogue";
	elseif (CLASS_SECONDARY == 5) then
		secondaryClassName = " / Priest";
	elseif (CLASS_SECONDARY == 6) then
		secondaryClassName = " / Death Knight";
	elseif (CLASS_SECONDARY == 7) then
		secondaryClassName = " / Shaman";
	elseif (CLASS_SECONDARY == 8) then
		secondaryClassName = " / Mage";
	elseif (CLASS_SECONDARY == 9) then
		secondaryClassName = " / Warlock";
	elseif (CLASS_SECONDARY == 11) then
		secondaryClassName = " / Druid";
	end

	CharacterLevelText:SetFormattedText(PLAYER_LEVEL, UnitLevel("player"), UnitRace("player"),
		UnitClass("player") .. secondaryClassName);
...
function PaperDollFrame_SetStat(statFrame, statIndex)
	...
	if ((posBuff == 0) and (negBuff == 0)) then
		...
		if (posBuff > 0) then
			tooltipText = tooltipText .. FONT_COLOR_CODE_CLOSE .. GREEN_FONT_COLOR_CODE .. "+" ..
				posBuff .. FONT_COLOR_CODE_CLOSE;
		end
		if (negBuff < 0) then
	...
	elseif (statIndex == 3) then
		...
		statFrame.tooltip2 = format(statFrame.tooltip2,
	...
	elseif (statIndex == 2) then
		-- local attackPower = GetAttackPowerForStat(statIndex, effectiveStat);
		--CRAFTCRAFT make values proper for crit
		local holySchool = 2;
		...
		minCrit = format("%.2f%%", minCrit - GetCombatRatingBonus(CR_CRIT_SPELL));
		statFrame.minCrit = minCrit;
		statFrame.tooltip2 = format(statFrame.tooltip2, GetCritChance("player") - GetCombatRatingBonus(CR_CRIT_MELEE),
			minCrit);
	elseif (statIndex == 4) then
		local baseInt = min(20, effectiveStat);
		local moreInt = effectiveStat - baseInt

		statFrame.tooltip2 = format(statFrame.tooltip2, baseInt + moreInt * MANA_PER_INTELLECT,
			moreInt);
	...
	elseif (statIndex == 5) then
		...
		statFrame.tooltip2 = format(statFrame.tooltip2, GetUnitHealthRegenRateFromSpirit("player"),
			GetUnitHealthRegenRateFromSpirit("player") * 3);
		if (UnitHasMana("player") or false) then
...
function PaperDollFrame_SetRating(statFrame, ratingIndex)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, statName) .. " " .. rating .. FONT_COLOR_CODE_CLOSE;
	...
	if (ratingIndex == CR_HIT_MELEE) then
		statFrame.tooltip2 = format(CR_HIT_MELEE_TOOLTIP, UnitLevel("player"), ratingBonus,
			GetCombatRating(CR_ARMOR_PENETRATION), GetArmorPenetration());
	...
	elseif (ratingIndex == CR_HIT_SPELL) then
		...
		statFrame.tooltip2 = format(CR_HIT_SPELL_TOOLTIP, UnitLevel("player"), ratingBonus, spellPenetration,
			spellPenetration);
...
function PaperDollFrame_SetResistances()
	for i = 1, NUM_RESISTANCE_TYPES, 1 do
		...
		frame.tooltipSubtext = format(RESISTANCE_TOOLTIP_SUBTEXT, _G["RESISTANCE_TYPE" .. frame:GetID()], unitLevel,
			resistanceLevel);
...
function PaperDollFrame_SetDefense(statFrame, unit)
	...
	statFrame.tooltip2 = format(DEFAULT_STATDEFENSE_TOOLTIP, GetCombatRating(CR_DEFENSE_SKILL),
...
function PaperDollFrame_SetDodge(statFrame)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		...
		string.format("%.02f", chance) .. "%" .. FONT_COLOR_CODE_CLOSE;
...
function PaperDollFrame_SetBlock(statFrame)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		...
		string.format("%.02f", chance) .. "%" .. FONT_COLOR_CODE_CLOSE;
	statFrame.tooltip2 = format(CR_BLOCK_TOOLTIP, GetCombatRating(CR_BLOCK), GetCombatRatingBonus(CR_BLOCK),
		GetShieldBlock());
	statFrame:Show();
end
...
function PaperDollFrame_SetResilience(statFrame)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, STAT_RESILIENCE) .. " " .. minResilience .. FONT_COLOR_CODE_CLOSE;
	statFrame.tooltip2 = format(RESILIENCE_TOOLTIP, lowestRatingBonus,
		min(lowestRatingBonus * RESILIENCE_CRIT_CHANCE_TO_DAMAGE_REDUCTION_MULTIPLIER, maxRatingBonus),
...
function PaperDollFrame_SetDamage(statFrame, unit)
	...
	local percent;
	minDamage, maxDamage, minOffHandDamage, maxOffHandDamage, physicalBonusPos, physicalBonusNeg, percent = UnitDamage(
		unit);
...
function PaperDollFrame_SetAttackSpeed(statFrame, unit)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, ATTACK_SPEED) .. " " .. text .. FONT_COLOR_CODE_CLOSE;
	statFrame.tooltip2 = format(CR_HASTE_RATING_TOOLTIP, GetCombatRating(CR_HASTE_MELEE),
...
function PaperDollFrame_SetAttackPower(statFrame, unit)
	...
	statFrame.tooltip2 = format(MELEE_ATTACK_POWER_TOOLTIP,
...
function PaperDollFrame_SetAttackBothHands(statFrame, unit)
	...
	if (mainHandAttackMod == 0) then
		...
		local color = RED_FONT_COLOR_CODE;
		statFrame.weaponSkill = COMBAT_RATING_NAME1 ..
			" " .. (mainHandAttackBase + mainHandAttackMod) .. " (" .. mainHandAttackBase .. color .. " " ..
			mainHandAttackMod .. ")";
		if (mainHandAttackMod > 0) then
			color = GREEN_FONT_COLOR_CODE;
			statFrame.weaponSkill = COMBAT_RATING_NAME1 ..
				" " ..
				(mainHandAttackBase + mainHandAttackMod) ..
				" (" .. mainHandAttackBase .. color .. " +" .. mainHandAttackMod .. FONT_COLOR_CODE_CLOSE .. ")";
		end
	end
	...
	if (total > 0) then
		statFrame.weaponRating = statFrame.weaponRating ..
			format(WEAPON_SKILL_RATING_BONUS,
				GetCombatRatingBonus(CR_WEAPON_SKILL) + GetCombatRatingBonus(CR_WEAPON_SKILL_MAINHAND));
	end
	...
	if (offhandSpeed) then
		if (offHandAttackMod == 0) then
			...
			local color = RED_FONT_COLOR_CODE;
			statFrame.offhandSkill = COMBAT_RATING_NAME1 ..
				" " ..
				(offHandAttackBase + offHandAttackMod) ..
				" (" .. offHandAttackBase .. color .. " " .. offHandAttackMod ..
				")";
			if (offHandAttackMod > 0) then
				color = GREEN_FONT_COLOR_CODE;
				statFrame.offhandSkill = COMBAT_RATING_NAME1 ..
					" " ..
					(offHandAttackBase + offHandAttackMod) ..
					" (" .. offHandAttackBase .. color .. " +" .. offHandAttackMod .. FONT_COLOR_CODE_CLOSE .. ")";
			end
		end
		...
		if (total > 0) then
			statFrame.offhandRating = statFrame.offhandRating ..
				format(WEAPON_SKILL_RATING_BONUS,
					GetCombatRatingBonus(CR_WEAPON_SKILL) + GetCombatRatingBonus(CR_WEAPON_SKILL_OFFHAND));
		end
	else
...
function PaperDollFrame_SetRangedAttack(statFrame, unit)
	...
	if (rangedAttackMod == 0) then
		...
		statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
			format(PAPERDOLLFRAME_TOOLTIP_FORMAT, COMBAT_RATING_NAME1) ..
			" " .. rangedAttackBase .. FONT_COLOR_CODE_CLOSE;
	else
		local color = RED_FONT_COLOR_CODE;
		if (rangedAttackMod > 0) then
			color = GREEN_FONT_COLOR_CODE;
			statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
				format(PAPERDOLLFRAME_TOOLTIP_FORMAT, COMBAT_RATING_NAME1) ..
				" " ..
				(rangedAttackBase + rangedAttackMod) ..
				" (" .. rangedAttackBase .. color .. " +" .. rangedAttackMod ..
	...
	if (total > 0) then
		statFrame.tooltip2 = statFrame.tooltip2 ..
			format(WEAPON_SKILL_RATING_BONUS,
				GetCombatRatingBonus(CR_WEAPON_SKILL) + GetCombatRatingBonus(CR_WEAPON_SKILL_RANGED));
	end
	statFrame:Show();
end
...
function PaperDollFrame_SetRangedAttackSpeed(statFrame, unit)
	...
	PaperDollFrame_SetLabelAndText(statFrame, WEAPON_SPEED, text);
	statFrame.tooltip2 = format(CR_HASTE_RATING_TOOLTIP, GetCombatRating(CR_HASTE_RANGED),
...
function PaperDollFrame_SetMeleeCritChance(statFrame)
	...
	local text = _G[statFrame:GetName() .. "StatText"];
	local critChance = GetCritChance("player") -- + GetCombatRatingBonus(CR_CRIT_MELEE);
	critChance = format("%.2f%%", critChance);
	text:SetText(critChance);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, MELEE_CRIT_CHANCE) .. " " .. critChance .. FONT_COLOR_CODE_CLOSE;
	statFrame.tooltip2 = format(CR_CRIT_MELEE_TOOLTIP, GetCombatRating(CR_CRIT_MELEE),
...
function PaperDollFrame_SetRangedCritChance(statFrame)
	...
	local text = _G[statFrame:GetName() .. "StatText"];
	local critChance = GetCritChance("player") -- + GetCombatRatingBonus(CR_CRIT_RANGED);
	critChance = format("%.2f%%", critChance);
	text:SetText(critChance);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, RANGED_CRIT_CHANCE) .. " " .. critChance .. FONT_COLOR_CODE_CLOSE;
	statFrame.tooltip2 = format(CR_CRIT_RANGED_TOOLTIP, GetCombatRating(CR_CRIT_RANGED),
...
function PaperDollFrame_SetSpellBonusHealing(statFrame)
	...
	local text = _G[statFrame:GetName() .. "StatText"];
	local bonusHealing = GetCombatRating(CR_HIT_TAKEN_MELEE);
	text:SetText(bonusHealing);
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE .. BONUS_HEALING .. FONT_COLOR_CODE_CLOSE;
	statFrame.tooltip2 = format(BONUS_HEALING_TOOLTIP, 100 * bonusHealing / (bonusHealing + 100));
	statFrame:Show();
end
...
function PaperDollFrame_SetSpellPenetration(statFrame)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, SPELL_PENETRATION) .. FONT_COLOR_CODE_CLOSE;
...
function PaperDollFrame_SetManaRegen(statFrame)
	...
	local text = _G[statFrame:GetName() .. "StatText"];
	if (not UnitHasMana("player") and not false) then
		text:SetText(NOT_APPLICABLE);
		statFrame.tooltip = nil;
		return;
	end
...
function PaperDollFrame_SetExpertise(statFrame, unit)
	...
	statFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, _G["COMBAT_RATING_NAME" .. CR_EXPERTISE]) ..
	...
	statFrame.tooltip2 = format(CR_EXPERTISE_TOOLTIP, text, GetCombatRating(CR_EXPERTISE),
...
function CharacterSpellBonusDamage_OnEnter(self)
	...
	GameTooltip:SetText(HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, BONUS_DAMAGE) .. " " .. self.minModifier .. FONT_COLOR_CODE_CLOSE);
	for i = 2, MAX_SPELL_SCHOOLS do
		GameTooltip:AddDoubleLine(_G["DAMAGE_SCHOOL" .. i], self.bonusDamage[i], NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR
			.g,
			NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
...
function CharacterSpellCritChance_OnEnter(self)
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
	GameTooltip:SetText(HIGHLIGHT_FONT_COLOR_CODE ..
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, COMBAT_RATING_NAME11) ..
		" " .. GetCombatRating(11) .. FONT_COLOR_CODE_CLOSE);
	local spellCrit;
	for i = 2, MAX_SPELL_SCHOOLS do
		spellCrit = format("%.2f", self.spellCrit[i]);
		spellCrit = spellCrit .. "%";
		GameTooltip:AddDoubleLine(_G["DAMAGE_SCHOOL" .. i], spellCrit, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g,
			NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
		GameTooltip:AddTexture("Interface\\PaperDollInfoFrame\\SpellSchoolIcon" .. i);
	end
	GameTooltip:Show();
...
function PaperDollItemSlotButton_OnEnter(self)
	...
	if (PaperDollFrameItemFlyout:IsShown()) then
		GameTooltip:SetOwner(PaperDollFrameItemFlyoutButtons, "ANCHOR_RIGHT", 6,
			-PaperDollFrameItemFlyoutButtons:GetHeight() - 6);
	else
		GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
	end
...
function CharacterAttackFrame_OnEnter(self)
	...
	if (self.offhandSkill) then
		GameTooltip:AddLine("\n");
		GameTooltip:AddLine(INVTYPE_WEAPONOFFHAND, HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR
			.b);
...
function CharacterDamageFrame_OnEnter(self)
	...
	if (self == PetDamageFrame) then
		GameTooltip:SetText(INVTYPE_WEAPONMAINHAND_PET, HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g,
			HIGHLIGHT_FONT_COLOR.b);
	else
		GameTooltip:SetText(INVTYPE_WEAPONMAINHAND, HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g,
			HIGHLIGHT_FONT_COLOR.b);
	end
	GameTooltip:AddDoubleLine(format(STAT_FORMAT, ATTACK_SPEED_SECONDS), format("%.2f", self.attackSpeed),
		NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g,
		NORMAL_FONT_COLOR.b);
	GameTooltip:AddDoubleLine(format(STAT_FORMAT, DAMAGE), self.damage, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g,
		NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
	GameTooltip:AddDoubleLine(format(STAT_FORMAT, DAMAGE_PER_SECOND), format("%.1f", self.dps), NORMAL_FONT_COLOR.r,
		NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
	-- Check for offhand weapon
	if (self.offhandAttackSpeed) then
		GameTooltip:AddLine("\n");
		GameTooltip:AddLine(INVTYPE_WEAPONOFFHAND, HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR
			.b);
		GameTooltip:AddDoubleLine(format(STAT_FORMAT, ATTACK_SPEED_SECONDS), format("%.2f", self.offhandAttackSpeed),
			NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g,
			NORMAL_FONT_COLOR.b);
		GameTooltip:AddDoubleLine(format(STAT_FORMAT, DAMAGE), self.offhandDamage, NORMAL_FONT_COLOR.r,
			NORMAL_FONT_COLOR.g,
			NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b);
		GameTooltip:AddDoubleLine(format(STAT_FORMAT, DAMAGE_PER_SECOND), format("%.1f", self.offhandDps),
			NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g,
			NORMAL_FONT_COLOR.b);
	end
	GameTooltip:Show();
...
function PaperDollFrameItemFlyout_CreateButton()
	...
	local button = CreateFrame("BUTTON", "PaperDollFrameItemFlyoutButtons" .. numButtons + 1, buttonAnchor,
	...
	if (math.floor(pos) == pos) then
		...
		button:SetPoint("TOPLEFT", buttonAnchor, "TOPLEFT", PDFITEMFLYOUT_BORDERWIDTH,
			-PDFITEMFLYOUT_BORDERWIDTH - (PDFITEM_HEIGHT - PDFITEM_YOFFSET) * pos);
	else
...
function PaperDollFrameItemFlyout_Show(paperDollItemSlot)
	...
	buttonAnchor:SetWidth((horizontalItems * PDFITEM_WIDTH) + ((horizontalItems - 1) * PDFITEM_XOFFSET) +
		PDFITEMFLYOUT_BORDERWIDTH);
	buttonAnchor:SetHeight(PDFITEMFLYOUT_HEIGHT +
		(math.floor((numItems - 1) / PDFITEMFLYOUT_ITEMS_PER_ROW) * (PDFITEM_HEIGHT - PDFITEM_YOFFSET)));
...
function PaperDollFrameItemFlyout_DisplayButton(button, paperDollItemSlot)
	...
	if (textureName) then
		...
		button.UpdateTooltip = function()
			GameTooltip:SetOwner(PaperDollFrameItemFlyoutButtons, "ANCHOR_RIGHT", 6,
				-PaperDollFrameItemFlyoutButtons:GetHeight() - 6); setTooltip();
		end;
		...
		button.UpdateTooltip =
			function()
				...
				if (SHOW_NEWBIE_TIPS == "1") then
					GameTooltip:AddLine(NEWBIE_TOOLTIP_EQUIPMENT_MANAGER_IGNORE_SLOT, NORMAL_FONT_COLOR.r,
						NORMAL_FONT_COLOR.g,
	...
	elseif (location == PDFITEMFLYOUT_UNIGNORESLOT_LOCATION) then
		...
		button.UpdateTooltip =
			function()
				...
				if (SHOW_NEWBIE_TIPS == "1") then
					GameTooltip:AddLine(NEWBIE_TOOLTIP_EQUIPMENT_MANAGER_UNIGNORE_SLOT, NORMAL_FONT_COLOR.r,
						NORMAL_FONT_COLOR.g,
						NORMAL_FONT_COLOR.b, 1);
				end
				GameTooltip:Show();
	...
	elseif (location == PDFITEMFLYOUT_PLACEINBAGS_LOCATION) then
		...
		button.UpdateTooltip =
			function()
				...
				if (SHOW_NEWBIE_TIPS == "1") then
					GameTooltip:AddLine(NEWBIE_TOOLTIP_EQUIPMENT_MANAGER_PLACE_IN_BAGS, NORMAL_FONT_COLOR.r,
						NORMAL_FONT_COLOR.g,
						NORMAL_FONT_COLOR.b, 1);
				end
				GameTooltip:Show();
...
function GearManagerDialogPopup_OnLoad(self)
	...
	local button = CreateFrame("CheckButton", "GearManagerDialogPopupButton1", GearManagerDialogPopup,
	...
	for i = 2, NUM_GEARSET_ICONS_SHOWN do
		button = CreateFrame("CheckButton", "GearManagerDialogPopupButton" .. i, GearManagerDialogPopup,
			"GearSetPopupButtonTemplate");
		button:SetID(i);
...
function RecalculateGearManagerDialogPopup()
	...
	if (popup.selectedTexture) then
		...
		FauxScrollFrame_OnVerticalScroll(GearManagerDialogPopupScrollFrame, offset * GEARSET_ICON_ROW_HEIGHT,
			GEARSET_ICON_ROW_HEIGHT, nil);
	end
	GearManagerDialogPopup_Update();
end
...
function GearManagerDialogPopup_Update()
	...
	-- Scrollbar stuff
	FauxScrollFrame_Update(GearManagerDialogPopupScrollFrame, ceil(_TotalItems / NUM_GEARSET_ICONS_PER_ROW),
		NUM_GEARSET_ICON_ROWS, GEARSET_ICON_ROW_HEIGHT);
end
...
function PlayerTitleFrame_UpdateTitles()
	...
	if (titleCount < 2) then
		...
		HybridScrollFrame_Update(PlayerTitlePickerScrollFrame, titleCount * PLAYER_TITLE_HEIGHT,
			PlayerTitlePickerScrollFrame:GetHeight());
