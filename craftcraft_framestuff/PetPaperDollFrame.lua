...
function PetPaperDollFrame_UpdateTabs()
	...
	if (HasPetUI()) then
		...
		PetPaperDollFrameTab1:SetPoint(currRef.point, currRef.relativeTo, currRef.relativePoint, currRef.xoffset,
			currRef.yoffset)
		currVal = currVal + 1;
	...
	if (GetNumCompanions("CRITTER") > 0) then
		...
		PetPaperDollFrameTab2:SetPoint(currRef.point, currRef.relativeTo, currRef.relativePoint, currRef.xoffset,
			currRef.yoffset);
		currVal = currVal + 1;
		currRef = tabPoints[currVal];
	...
	if (GetNumCompanions("MOUNT") > 0) then
		...
		PetPaperDollFrameTab3:SetPoint(currRef.point, currRef.relativeTo, currRef.relativePoint, currRef.xoffset,
			currRef.yoffset);
		currVal = currVal + 1;
	else
...
function PetPaperDollFrame_OnEvent(self, event, ...)
	...
	elseif (event == "COMPANION_LEARNED") then
		...
		PetPaperDollFrame_SetCompanionPage((PetPaperDollFrameCompanionFrame.mode == "MOUNT") and
		PetPaperDollFrameCompanionFrame.pageMount or PetPaperDollFrameCompanionFrame.pageCritter);
...
function PetPaperDollFrame_FindCompanionIndex(creatureID, mode)
	...
	if (not creatureID) then
		creatureID = (PetPaperDollFrameCompanionFrame.mode == "MOUNT") and PetPaperDollFrameCompanionFrame.idMount or
		PetPaperDollFrameCompanionFrame.idCritter;
	end
...
function CompanionSummonButton_OnClick()
	...
	local creatureID, creatureName, spellID, icon, active = GetCompanionInfo(PetPaperDollFrameCompanionFrame.mode,
		selected);
	if (active) then
...
function PetPaperDollFrame_UpdateCompanions()
	...
	if (selected > 0) then
		...
		if (active and creatureID) then
			CompanionSummonButton:SetText(PetPaperDollFrameCompanionFrame.mode == "MOUNT" and BINDING_NAME_DISMOUNT or
			PET_DISMISS);
		else
...
function PetPaperDollFrame_UpdateCompanionCooldowns()
	...
	for i = 1, NUM_COMPANIONS_PER_PAGE do
		...
		if (button.creatureID) then
			local start, duration, enable = GetCompanionCooldown(PetPaperDollFrameCompanionFrame.mode,
				offset + button:GetID());
...
function PetPaperDollFrame_SetStats()
	for i = 1, NUM_PET_STATS, 1 do
		...
		format(PAPERDOLLFRAME_TOOLTIP_FORMAT, _G["SPELL_STAT" .. i .. "_NAME"]) .. " ";
		...
		if ((posBuff == 0) and (negBuff == 0)) then
			...
			if (posBuff > 0) then
				...
				FONT_COLOR_CODE_CLOSE .. GREEN_FONT_COLOR_CODE .. "+" .. posBuff .. FONT_COLOR_CODE_CLOSE;
			end
			if (negBuff < 0) then
		...
		elseif (i == 5) then
			frame.tooltip2 = format(frame.tooltip2, GetUnitHealthRegenRateFromSpirit("pet"),
				GetUnitHealthRegenRateFromSpirit("pet"));
			if (UnitHasMana("pet")) then
...
function PetPaperDollFrame_SetSpellBonusDamage()
	...
	PetSpellDamageFrame.tooltip = HIGHLIGHT_FONT_COLOR_CODE ..
	format(PAPERDOLLFRAME_TOOLTIP_FORMAT, SPELL_BONUS) .. FONT_COLOR_CODE_CLOSE .. " " .. spellDamageBonusText;
