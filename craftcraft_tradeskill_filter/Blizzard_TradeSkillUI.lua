...
function ShouldSkip(id, skillName, skillType)
	if skillType == "header" then
		return TradeSkillFrame.hide_header
	end
	...
	local skip = false
	for _, value in pairs(TradeSkillFrame.filter_menu.submenus) do
		if (value:Filter(itemLink)) then
			skip = true
		end
	end
...
function TradeSkillFrame_Update()
	...
	-- ScrollFrame update
	FauxScrollFrame_Update(TradeSkillListScrollFrame, numTradeSkills, TRADE_SKILLS_DISPLAYED, TRADE_SKILL_HEIGHT, nil,
		nil, nil, TradeSkillHighlightFrame, 293, 316);
	...
	local skip = 0
	local i = 1
	while i <= TRADE_SKILLS_DISPLAYED do
		skillIndex = i + skip + skillOffset;
		...
		if ShouldSkip(skillIndex, skillName, skillType) then -- Filter here
			skip = skip + 1
		else
			if (skillIndex <= numTradeSkills) then
				...
				if (skillType == "header") then
					...
					if (GetTradeSkillSelectionIndex() == skillIndex) then
						...
						skillButtonCount:SetVertexColor(HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g,
...
function TradeSkillFrame_PlaytimeUpdate()
	if (PartialPlayTime()) then
		...
		if (not TradeSkillCreateButtonMask:IsShown()) then
			...
			TradeSkillCreateButtonMask.tooltip = format(PLAYTIME_TIRED_ABILITY,
				REQUIRED_REST_HOURS - floor(GetBillingTimeRested() / 60));
		end

		TradeSkillCreateAllButton:Disable();
		if (not TradeSkillCreateAllButtonMask:IsShown()) then
			TradeSkillCreateAllButtonMask:Show();
			TradeSkillCreateAllButtonMask.tooltip = format(PLAYTIME_TIRED_ABILITY,
				REQUIRED_REST_HOURS - floor(GetBillingTimeRested() / 60));
		end
	elseif (NoPlayTime()) then
		...
		if (not TradeSkillCreateButtonMask:IsShown()) then
			TradeSkillCreateButtonMask:Show();
			TradeSkillCreateButtonMask.tooltip = format(PLAYTIME_UNHEALTHY_ABILITY,
				REQUIRED_REST_HOURS - floor(GetBillingTimeRested() / 60));
		end
		...
		if (not TradeSkillCreateAllButtonMask:IsShown()) then
			TradeSkillCreateAllButtonMask:Show();
			TradeSkillCreateAllButtonMask.tooltip = format(PLAYTIME_UNHEALTHY_ABILITY,
				REQUIRED_REST_HOURS - floor(GetBillingTimeRested() / 60));
		end
	else
